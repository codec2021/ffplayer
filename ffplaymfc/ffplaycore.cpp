...
const char program_name[] = "ffplaymfc2.0";
...
typedef struct VideoState 
{
	...
	//define the method of video display
...
//Customize an event to adjust the playback progress
#define FFMFC_SEEK_BAR_EVENT    (SDL_USEREVENT + 4)
int seek_bar_pos;
//A specially set flag, which will be set to 1 when the program is about to exit
...
int aframe_index = 0;
int packet_index = 0;
int vframe_total = 0;//add by codec2021,the total frames in this file.

int ffmfc_reset_index(void)
{
	vframe_index = 0;
	aframe_index = 0;
	packet_index = 0;
...
int ffmfc_param_global(VideoState *is)
{
	...
	//CString can be directly assigned to LPCTSTR
	...
	if(video_stream != -1)
	{
		...
		framerate_temp = (float)(pFormatCtx->streams[video_stream]->r_frame_rate.num)/(pFormatCtx->streams[video_stream]->r_frame_rate.den);
	...
	timelong_temp = (float)(pFormatCtx->duration)/1000000;

	//transform to hh:mm:ss
	int tns, thh, tmm, tss;
	tns  = (float)(pFormatCtx->duration)/1000000;
	thh  = tns / 3600;
	tmm  = (tns % 3600) / 60;
	tss  = (tns % 60);
	...
	//MetaData
	//Obtained from AVDictionary
	//Need to use AVDictionaryEntry object
...
int ffmfc_param_packet(VideoState *is,AVPacket *packet)
{
	...
	//The size of the stream unit, such as NAL in H.264
...
int ffmfc_param_vframe(VideoState *is,AVFrame *pFrame,AVPacket *packet)
{
	...
	//add by codec2021
	CString pkt_duration;
	CString pkt_size;
	CString real_time;
	CString real_time_diff;
	...
	{
		dlg->OnBnClickedPause();//pause.
		//dlg->SystemClear();//
	}

	f_index.Format(_T("%d"),vframe_index + 1);
	...
	//vframe_index cannot be assigned directly! Be sure to use f_index to execute Format! and then assign!
	...
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex,2,pts);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex,3,dts);

	pkt_size.Format(_T("%d"), pFrame->pkt_size);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 4, pkt_size);

	//insert pkt_pts_time = packet->pts/90000,add by codec2021
	int64_t ptime = (float)pFrame->pkt_pts / 90000.0 * 1000;
	real_time.Format(_T("%d"), ptime);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 5, real_time);


	real_time_diff.Format(_T("%d"), ptime - time_diff > 1000?-1:(ptime - time_diff));
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 6, real_time_diff);
	time_diff = ptime;
	...
	*/
	...
	if (pFormatCtx->streams[video_stream]->r_frame_rate.den != 0 && pFormatCtx->streams[video_stream]->r_frame_rate.num != 0)
	{
		frame_rate = pFormatCtx->streams[video_stream]->r_frame_rate.num / (double)pFormatCtx->streams[video_stream]->r_frame_rate.den;
	}
	else
	{
		frame_rate = -1;
	}
	video_rate.Format(_T("%.2f"), frame_rate);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 7, video_rate);



	frame_w.Format(_T("%d"), pFrame->width);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 8, frame_w);

	frame_h.Format(_T("%d"), pFrame->height);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 9, frame_h);

#if 0
	//add by codec2021
	int64_t avg_bit_rate = 0;
	avg_bit_rate = is->video_st->codec->bit_rate;
	bit_rate.Format(_T("%d"), avg_bit_rate);
	dlg->vddlg->m_videodecodelist.SetItemText(nIndex, 8, bit_rate);
#endif
...
int ffmfc_param_aframe(VideoState *is,AVFrame *pFrame,AVPacket *packet)
{
	...
	CString pkt_duration;
	CString time_stamp;
	CString time_stamp_diff;
	static int64_t time_diff;

	f_index.Format(_T("%d"),aframe_index + 1);
	...
	pkt_dts.Format(_T("%d"), packet->dts);
	pkt_duration.Format(_T("%d"), packet->duration);

	time_stamp.Format(_T("%d"), packet->dts/90);
	time_stamp_diff.Format(_T("%d"), abs(packet->dts/90 - time_diff) > 1000? -1:abs(packet->dts/90 - time_diff) );
	
	time_diff = packet->dts/90;
	...
	dlg->addlg->m_audiodecodelist.SetItemText(nIndex,3,pkt_dts);

	dlg->addlg->m_audiodecodelist.SetItemText(nIndex,4,pkt_duration);
	dlg->addlg->m_audiodecodelist.SetItemText(nIndex,5,time_stamp);

	dlg->addlg->m_audiodecodelist.SetItemText(nIndex,6,time_stamp_diff);
...
static int packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
	...
	//PacketList composes Packets into a linked list, and each PacketList has only one Packet
...
//Calculate the position of the display window (keep filling the screen)
...
static int video_open(VideoState *is, int force_set_video_mode)
{
	...
	if (screen && is->width == screen->w 
		&& screen->w == w && is->height == screen->h 
		&& screen->h == h && !force_set_video_mode)
	{
		return 0;
	}
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts1, int64_t pos)
{
	...
	* pts1 is the dts of the pkt / pts of the frame */
	...
	while (is->pictq_size >= VIDEO_PICTURE_QUEUE_SIZE - 2 && !is->videoq.abort_request) 
	{
	...
	if (!vp->bmp || vp->reallocate || !vp->allocated || vp->width  != src_frame->width || vp->height != src_frame->height) 
	{
			SDL_Event event;
			vp->allocated  = 0;
			vp->reallocate = 0;
	...
	if (vp->bmp) 
	{
		...
		av_picture_copy(&pict, (AVPicture *)src_frame,src_frame->format, vp->width, vp->height);
#else
...
static int get_video_frame(VideoState *is, AVFrame *frame, int64_t *pts, AVPacket *pkt)
{
	...
	if (got_picture) 
	{
		//set video decode param.
		ffmfc_param_vframe(is,frame,pkt);

		int ret = 1;
		if (decoder_reorder_pts == -1) 
		{
			//compute the pts for display.
			...
			(framedrop>0 || (framedrop && is->audio_st))) 
		{
			...
			if (is->frame_last_pts != AV_NOPTS_VALUE && *pts) 
			{
				...
				if (fabs(clockdiff) < AV_NOSYNC_THRESHOLD && ptsdiff > 0 && ptsdiff < AV_NOSYNC_THRESHOLD &&
...
static int stream_component_open(VideoState *is, int stream_index)
{
	...
	switch(avctx->codec_type)
	{
		case AVMEDIA_TYPE_AUDIO   : 
			is->last_audio_stream = stream_index; if(audio_codec_name   ) codec= avcodec_find_decoder_by_name(   audio_codec_name); 
			break;
		case AVMEDIA_TYPE_SUBTITLE: 
			is->last_subtitle_stream = stream_index; if(subtitle_codec_name) codec= avcodec_find_decoder_by_name(subtitle_codec_name); 
			break;
		case AVMEDIA_TYPE_VIDEO   : 
			is->last_video_stream = stream_index; if(video_codec_name   ) codec= avcodec_find_decoder_by_name(   video_codec_name); 
			break;
	}
	if (!codec)
	{
...
static int read_thread(void *arg)
{
	...
	//如果此函数耗时太长需要优化：更改max_analyze_duration=1000及probesize=2048，效果非常明显
	//ic->probesize = 2048;
	...
	float fps;
	//fps = (ic->streams[video_index]->codec->time_base.den)/(ic->streams[video_index]->codec->time_base.num);
	//printf("视频帧率：%f\n",fps);
	if (!audio_disable)
	...
	for (;;) 
	{
		...
		if (eof) 
		{
			...
			if (is->audioq.size + is->videoq.size + is->subtitleq.size == 0) 
			{
				...
				{
					//printf("the total frames is %d\n",vframe_index);
					vframe_total = vframe_index;//add by codec2021
...
{
	//add by codec2021,防止在没有播放视频的时候，设置了参数
	if (g_is == NULL)
	{
		return;
	}
...
{
	//add by codec2021,防止在没有播放视频的时候，设置了窗口缩放
	if (g_is == NULL)
	{
		return;
	}
	...
	SDL_Event event;
	event.type = SDL_VIDEORESIZE;
	event.resize.w = w * percentage*0.01;
	event.resize.h = h * percentage*0.01;
	SDL_PushEvent(&event);
}
...
void ffmfc_audio_display(int mode)
{
	...
	switch(mode)
	{
		case 0:event.key.keysym.sym=SDLK_w;
			break;
		case 1:event.key.keysym.sym=SDLK_e;
			break;
		case 2:event.key.keysym.sym=SDLK_r;
			break;
	}
	SDL_PushEvent(&event);
}
...
void ffmfc_seek(int time)
{
	...
	switch (time)
	{
	case -10 :event.key.keysym.sym = SDLK_LEFT;
		break;
	case 10 :event.key.keysym.sym  = SDLK_RIGHT;
		break;
	case -60 :event.key.keysym.sym = SDLK_DOWN;
		break;
	case 60 :event.key.keysym.sym  = SDLK_UP;
		break;
	case -600 :event.key.keysym.sym = SDLK_PAGEDOWN;
		break;
	case 600 :event.key.keysym.sym  = SDLK_PAGEUP;
		break;
	default :event.key.keysym.sym   = SDLK_RIGHT;
		break;
	}
...
static void toggle_audio_display(VideoState *is,int mode)
{
	...
	switch(mode)
	{
		case SHOW_MODE_VIDEO:is->show_mode = (VideoState::ShowMode)SHOW_MODE_VIDEO;
			break;
		case SHOW_MODE_WAVES:is->show_mode = (VideoState::ShowMode)SHOW_MODE_WAVES;
			break;
		case SHOW_MODE_RDFT:is->show_mode = (VideoState::ShowMode)SHOW_MODE_RDFT;
			break;
...
int ffmfc_play(LPVOID lpParam)
{
	...
	{
		AfxMessageBox(_T("input_filename malloc fail!"));
	}
	//将指定窗口的标题栏的文字拷贝到缓冲区内
